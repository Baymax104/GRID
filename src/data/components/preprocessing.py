...
def map_sparse_id_to_semantic_id(
    row: dict[str, torch.Tensor],
    semantic_id_bundle: ModelOutput,
    features_to_apply: list[str] | None = None,
    ...
    Args:
        row: dataset row
        semantic_id_bundle: the model output containing semantic ids
        features_to_apply: feature key list for applying function
    ...
    for k, v in row.items():
        if is_feature_in_features_to_apply(features_to_apply, k):
            ...
            semantic_ids = gather_predictions_by_keys(semantic_id_bundle, v)
...
def map_sparse_id_to_embedding(
    ...
    row[embedding_field_to_add] = gather_predictions_by_keys(embedding_bundle, row[sparse_id_field]).squeeze()
    return row
