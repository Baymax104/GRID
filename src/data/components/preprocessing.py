...
def map_sparse_id_to_semantic_id(
    ...
    **kwargs,
    ...
    for k, v in row.items():
        if is_feature_in_features_to_apply(features_to_apply, k):
            id_bundle = dataset_config.semantic_id_map.get(k, None)
            ...
            if id_bundle is not None:
                semantic_ids = lookup_values_in_keyed_prediction_bundle(id_bundle, v)
...
def map_sparse_id_to_embedding(
    ...
    **kwargs,
    ...
    embedding_bundle = dataset_config.embedding_map.get(sparse_id_field, None)
