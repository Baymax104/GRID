...
class ResidualQuantizationVAE(LightningModule):
    ...
    def _compute_initial_centroids(self, layer_idx: int, buffer: torch.Tensor) -> None:
        ...
        self.init_centroids_list[layer_idx] = centroids.detach()
    ...
    def forward(self, embeddings: torch.Tensor):
        ...
        - Layer N trains once layer N-1 is initialized or in initial step
    ...
    def model_step(self, model_input: ItemBatch):
        ...
        if (
            ...
            and self.is_initialized_list[-1]
        ):
    ...
    def training_step(self, model_input: ItemBatch) -> torch.Tensor:
        ...
        if self.training_loop_function is not None:
            is_initialized = self.is_initialized_list[-1]
