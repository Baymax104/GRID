...
- `*_flat.sh`：当前仓库里最接近“可执行真相”的启动命令模板，已经统一成 `uv run torchrun ...`。
...
- 现成脚本：
  - `sem_embeds_inference_flat.sh`
  - `rkmeans_train_flat.sh`
  - `rkmeans_inference_flat.sh`
  - `tiger_train_flat.sh`

## 实际 pipeline 顺序
- 语义向量：`experiment=sem_embeds_inference_flat`
- 训练量化器：`experiment=rkmeans_train_flat`
- 生成 semantic IDs：`experiment=rkmeans_inference_flat`
- 训练生成推荐模型：`experiment=tiger_train_flat`
- 生成推荐结果：`experiment=tiger_inference_flat`
...
- `configs/inference.yaml` 的 `ckpt_path` 默认是必填（`???`）；只有像 `sem_embeds_inference_flat` 这种显式覆盖成 `null` 的实验才能无 checkpoint 跑。
