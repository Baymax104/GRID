# one-based-epoch-display Specification

## Purpose
TBD - created by archiving change one-based-epoch-display. Update Purpose after archive.
## Requirements
### Requirement: Progress bar SHALL display epoch numbers starting at 1
系统在控制台 progress bar 中显示 epoch 时，SHALL 从 1 开始，而不是直接暴露内部 0-based epoch 值。
...
epoch 的 1-based 显示 SHALL 只影响日志展示层，SHALL NOT 改变训练内部语义。
...
默认 progress bar 在 train / validation / test / predict 场景中都 SHALL 使用同一套 epoch 显示规则。
