...
官方 data YAML 中的 dataset config block SHALL 统一通过 Hydra `_target_` 指向 `src.common.configs.data.DatasetConfig`。

#### Scenario: 官方 experiments 不引用旧 dataset config 类
- **WHEN** 维护者检查 `configs/data/*.yaml` 中的 dataset config `_target_`
- **THEN** `_target_` MUST 使用 `src.common.configs.data.DatasetConfig`
- **AND** `_target_` MUST NOT 使用 `SemanticIDDatasetConfig` 或 `ItemDatasetConfig`
- **AND** `_target_` MUST NOT 使用 `src.data.components.config_models.DatasetConfig`
