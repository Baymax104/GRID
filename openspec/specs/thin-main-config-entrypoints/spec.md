# thin-main-config-entrypoints Specification

## Purpose
TBD - created by archiving change thin-main-config-entrypoints. Update Purpose after archive.
## Requirements
### Requirement: Main config entrypoints SHALL remain thin
`train.yaml` 与 `inference.yaml` SHALL 作为 defaults 导入层与通用运行开关层，而不再承担 experiment 级手动输入入口职责。
...
`data_dir`、`ckpt_path` 等实验级手动输入 SHALL 统一由 experiment 顶层提供。
...
在主入口瘦身之后，官方 experiment 的配置合成与主链路行为 SHALL 保持可用。
