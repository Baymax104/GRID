...
默认训练与推理主链路在初始化 pipeline 时，SHALL 不再读取或依赖 restart metadata 文件，也 SHALL NOT 因为 restart callback 或 launcher 的存在与否而改变默认行为。
...
当配置显式启用目录解析最新 checkpoint 的能力时，系统 SHALL 继续支持从 checkpoint 目录中自动选择最近的 `.ckpt` 文件。
...
仓库保留的 restart 模块 SHALL 明确标注为废弃历史能力，说明其不属于默认主链路。
...
默认训练命令模板 SHALL NOT 继续包含已经失效的 restart 参数遗留。
