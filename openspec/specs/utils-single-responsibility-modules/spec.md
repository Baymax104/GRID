# utils-single-responsibility-modules Specification

## Purpose
TBD - created by archiving change reorganize-utils-package. Update Purpose after archive.
## Requirements
### Requirement: utils 包中每个模块 SHALL 保持单一职责
`src/utils/` 包中的每个模块 SHALL 只承载一类职责。禁止以 `utils.py` 或类似泛化命名的 catch-all 模块堆积不相关功能。

#### Scenario: 新增工具函数时选择正确模块
- **WHEN** 维护者需要在 utils 包中新增一个工具函数
- **THEN** 该函数 MUST 放入已存在的、职责匹配的模块
- **THEN** 若无匹配模块，MUST 新建一个职责命名的模块（如 `startup.py`、`model_utils.py`）
- **THEN** 该函数 MUST NOT 放入一个名称泛化的 catch-all 模块

#### Scenario: 启动预处理与模型操作分离
- **WHEN** 维护者检查 utils 包结构
- **THEN** 启动预处理函数（如 `extras`）MUST 位于 `startup.py`
- **THEN** 模型模块操作函数（如 `delete_module`、`reset_parameters`）MUST 位于 `model_utils.py`
...
- **THEN** 它 MUST 位于 `src/utils/model_utils.py`
- **THEN** 它 MUST NOT 位于 `tensor_utils.py`（该模块专用于 keyed prediction bundle 协议）
