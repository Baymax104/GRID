# row-based-data-iteration Specification

## Purpose
TBD - created by archiving change remove-batch-iterator-mode. Update Purpose after archive.
## Requirements
### Requirement: Official data pipeline SHALL use row-based iteration only
官方数据链路 SHALL 只支持逐样本迭代，SHALL NOT 继续暴露 batch iterator 模式。
...
官方 iterator 抽象与实现 SHALL NOT 继续暴露 `iter_batches()` 能力。
...
datamodule 在组装 DataLoader 时 SHALL 直接按 row-based 模式的语义设置批处理参数。
...
官方 experiment 配置 SHALL NOT 继续声明 `iterate_per_row` 参数。
