...
系统 SHALL 能够在不依赖 TensorFlow 运行时的前提下读取和解析 `.tfrecord.gz` 数据。
...
新的 TFRecord 解析输出 SHALL 保持对现有预处理链路的兼容性。
...
运行时代码中 SHALL NOT 继续保留 `import tensorflow` 依赖。
...
替换 TensorFlow 后，官方 pipeline SHALL 保持模型前输入契约不变。
