...
- **WHEN** 维护者查找 TIGER label function 输出、TIGER 模型输入或 item batch 容器 dataclass
...
- **AND** `data_models.py` MUST NOT 包含 `LabelFunctionOutput`、`SequentialModuleLabelData` 或 `SequentialModelInputData`
