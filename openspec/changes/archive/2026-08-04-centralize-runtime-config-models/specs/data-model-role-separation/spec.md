...
用于描述数据管线配置的 dataclass 与用于承载运行时 batch / label / model input 的 dataclass SHALL 位于不同模块，且不得继续混放在同一个文件中。数据配置 dataclass SHALL 位于 common config package，运行时批数据 dataclass SHALL 留在 data package。
