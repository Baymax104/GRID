...
The datamodule package SHALL expose the base, sequence, and item implementations from separate module files so module paths align with the new structure.
...
- **THEN** sequence experiments MUST reference `src.data.loading.datamodules.sequence.SequenceDataModule`
- **THEN** item-based experiments MUST reference `src.data.loading.datamodules.item.ItemDataModule`
- **THEN** configurations MUST NOT depend on the old combined module path
