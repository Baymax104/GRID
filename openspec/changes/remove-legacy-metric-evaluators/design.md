...
`Evaluator` 和 `SIDRetrievalEvaluator` 曾同时负责指标集合管理、输入转换和 reset/device 迁移。新框架已将这些职责拆分到 `MetricCallback`、`MetricEngine` 和 TIGER SID retrieval input adapter。
...
- [Risk] 外部未纳入仓库的脚本可能引用旧 evaluator。  
  Mitigation: 官方 configs 和源码均迁移到新 runtime；如需兼容外部脚本，可由调用方改用 `MetricEngine` 配置和 TIGER SID retrieval input adapter。
